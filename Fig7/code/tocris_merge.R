...
primary_hits <- read_rds(here("Fig2/data/z_scores.rds")) %>%
  ...
  select(-number) %>% 
  # scale from 0 to 1 such that the higher effect is 1
  group_by(assay) %>%
...
dr <- read_rds(here("Fig2/data/ec50.rds")) %>%
  ...
  # scale from 0 to 1 such that the higher effect is 1
  mutate(rescaled_value = scales::rescale(value, to = c(1, 0)))

no_dr <- primary_hits %>% 
  ungroup() %>% 
  filter(!treatment %in% dr$treatment) %>% 
  distinct(treatment) %>% 
  mutate(
    assay = "dose-response",
    stages = "Mf",
...
### adult data
motility <- read_rds(here("Fig6/data/motility_end.rds")) %>%
  rename(value = total_motility) %>%
  mutate(assay = "motility") %>% 
  # group_by(treatment, stages, assay) %>% 
  # summarise(value = mean(value)) %>% 
  group_by(stages) %>% 
  # scale from 0 to 1 such that the higher effect is 1
  mutate(rescaled_value = scales::rescale(value, to = c(1, 0)))

fecundity <- read_rds(here("Fig6/data/proportions.rds")) %>%
...
lactate <- read_rds(here("Fig6/data/lactate_data.rds")) %>%
  ungroup() %>%
...
write_rds(join, here('Fig7/subplots/heatmap.rds'))

save_plot(here("Fig7/subplots/Fig7b.pdf"), join,
          base_height = 8, base_width = 6)

save_plot(here("Fig7/subplots/Fig7b.png"), join,
          base_height = 8, base_width = 6)
...
f_hits <- read_rds(here('Fig6/data/fecundity_hits.rds')) %>% 
  mutate(stages = 'Adult female', phenotype = 'fecundity')
m_hits <- read_rds(here('Fig6/data/motility_hits.rds')) %>% 
  mutate(phenotype = 'motility')
l_hits <- read_rds(here('Fig6/data/metabolism_hits.rds')) %>% 
...
write_rds(upset, here('Fig7/subplots/upset.rds'))

save_plot(here("Fig7/subplots/Fig7a.pdf"), upset,
          base_height = 3, base_width = 6)

save_plot(here("Fig7/subplots/Fig7a.png"), upset,
          base_height = 3, base_width = 6)
