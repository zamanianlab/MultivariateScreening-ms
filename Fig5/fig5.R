...
a_motility <- read_rds(here('Fig5/subplots/optimized_motility.rds')) +
  theme(
    # axis.title.x = element_blank(),
    legend.position = 'empty')

b_metabolism <- read_rds(here('Fig5/subplots/heatkilled_metabolism.rds'))

c_fecundity <- read_rds(here('Fig5/subplots/optimized_fecundity.rds')) +
  theme(legend.position = 'empty')

d_fecundity <- read_rds(here('Fig5/subplots/baseline_fecundity.rds')) +
  theme(legend.position = 'right')

e_fecundity <- read_rds(here('Fig5/subplots/heatkilled_fecundity.rds'))

f_scheme <- image_read_pdf(here('Fig5/subplots/Fig5f.pdf'))
