...
a_motility <- read_rds(here('Fig6/subplots/motility.rds')) +
  theme(
    axis.title.x = element_blank(),
    legend.position = 'empty'
    )

b_distinct_lines <- read_rds(here('Fig6/subplots/distinct_lines.rds'))

c_fecundity <- read_rds(here('Fig6/subplots/proportions.rds')) +
  theme(
    legend.position = 'empty'
  )

c_inset <- read_rds(here('Fig6/subplots/baseline.rds')) +
  theme(
...
d_lactate <- read_rds(here('Fig6/subplots/lactate.rds')) +
  theme(
    legend.position = 'bottom'
  )

e_viability <- image_read_pdf(here('Fig6/subplots/Fig6e.pdf'))
...
fig6 <- plot_grid(left, right,
                  ncol = 2, rel_widths = c(1, 0.5))

save_plot(here('Fig6/Fig6.pdf'),
          fig6, base_width = 8.66, base_height = 11)
